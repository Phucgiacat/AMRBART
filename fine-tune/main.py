...
def main():
    ...
    if training_args.do_train and training_args.resume_from_checkpoint is None and last_checkpoint is None:
        print("Loading amr_encoder weights from encoder...")
        model.model.amr_encoder.load_state_dict(model.model.encoder.state_dict())
        for layer in model.model.decoder.layers:
            layer.amr_attn_layer_norm.load_state_dict(layer.encoder_attn_layer_norm.state_dict())
            layer.fusion_attn_layer_norm.load_state_dict(layer.encoder_attn_layer_norm.state_dict())
    ...
    # if training_args.do_train and training_args.resume_from_checkpoint is None and last_checkpoint is None:
    #     # smart_init nlr and nrl
    #     print("smart initializing <dfs_NLR> and <dfs_NRL>...")
    #     nlr_idx = tokenizer.encoder['<dfs_NLR>']
    #     nrl_idx = tokenizer.encoder['<dfs_NRL>']
    #     amr_idx = tokenizer.encoder['<AMR>']
    #     vec1 = model.model.shared.weight.data[amr_idx].detach().clone()
    #     vec2 = model.model.shared.weight.data[amr_idx].detach().clone()
    #     noise1 = torch.empty_like(vec1)
    #     noise1.uniform_(-0.1, +0.1)
    #     model.model.shared.weight.data[nlr_idx] = vec1 + noise1

    #     noise2 = torch.empty_like(vec2)
    #     noise2.uniform_(-0.1, +0.1)
    #     model.model.shared.weight.data[nrl_idx] = vec2 + noise2   
        ...
        if data_args.overwrite_cache or not os.path.exists(data_args.data_cache_dir + "/train"):
            with training_args.main_process_first(desc="train dataset map pre-processing"):
                train_dataset = train_dataset.map(
                    raw_datasets.tokenize_function_for_train,
                    batched=True,
    ...
    column_names = raw_datasets.datasets["validation"].column_names
        ...
        if data_args.overwrite_cache or not os.path.exists(data_args.data_cache_dir + "/test"):
            with training_args.main_process_first(desc="prediction dataset map pre-processing"):
                if data_args.predict_without_label:
                    ...
                    remove_cols = [c for c in raw_datasets.datasets["test"].column_names if c != "src"]
                else:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                if training_args.task == "text2amr":
                    ...
                    input_texts = []
                    if training_args.include_inputs_for_metrics and hasattr(predict_results, 'inputs') and predict_results.inputs is not None:
                        inputs = np.where(predict_results.inputs != -100, predict_results.inputs, tokenizer.pad_token_id)
                    ...
                    for idx in range(len(preds)):
                        ...
                        graph, status, _ = tokenizer.decode_amr(
                            ith_pred, restore_name_ops=False
                        )
                        graph.status = status
